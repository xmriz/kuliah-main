...
countCond :: [Int] -> Int
    {- menghitung banyaknya elemen yang dari list of integer l yang memenuhi :
...
countCond l
    | isEmpty l = 0
    | head l == 0 = 1 + countCond (tail l)
    | mod (head l) 2 /= 0 && mod (head l) 3 == 0 && mod (head l) 5 /= 0 = 1 + countCond (tail l) 
    | mod (head l) 2 == 0 && 51 <= head l && head l <= 100 = 1 + countCond (tail l)
    | otherwise = countCond (tail l)