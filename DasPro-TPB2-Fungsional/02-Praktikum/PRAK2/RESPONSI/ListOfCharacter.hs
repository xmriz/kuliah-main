...
-- isElmt e l true jika e adalah elemen list of character l
makeUnique :: [Char] -> [Char]
    {- fungsi makeUnique yang menerima masukan sebuah list of character, misalnya lc, dan menghasilkan list dengan elemen-elemen unik, yaitu, kemunculan setiap elemen (jika tidak kosong), hanya 1. -}

isElmt x l
   | isEmpty l = False
   | otherwise = 
         if x == last l then True
         else isElmt x (init l)

makeUnique l
   | isEmpty l = []
   | otherwise =
      if isElmt (last l) (init l) then makeUnique (init l)
      else konsDot (makeUnique (init l)) (last l)